...
//! facts.  The `ContractedDeductionGraph` returned by
//! `deduction_graph_from_sequents` contains an `inputs` `HashMap`
//! from predicate name to `FactInput, and an `outputs`
//! `DeductionSink`; one can insert (or remove) facts into any of
//! these `InputSession`s, `advance_all_fact_inputs` to make sure the
//! changes are visible and time has advanced, and the changes' impact
//! on the output `DeductionSink`s will eventually be visible.
//!
...
/// A `ContractedDeductionGraph` abstracts out a deduction dataflow
/// graph into its input and output arcs.
///
/// Any change to the `inputs` will, after advancing time, flushing
/// that information, and letting the timely worker run, update the
/// `outputs`.
pub struct ContractedDeductionGraph<T: Timestamp> {
    pub inputs: HashMap<String, FactInput<T>>,
    pub outputs: DeductionSink<T>,
}
...
) -> Result<ContractedDeductionGraph<T>, &'static str>
where
    G: Input<Timestamp = T>,
    T: Timestamp + Lattice + Ord,
{
    ...
    Ok(ContractedDeductionGraph {
        inputs: sources.inputs,
        outputs: results.into_sinks(),
    })
}
