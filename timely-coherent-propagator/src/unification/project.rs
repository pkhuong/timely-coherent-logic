...
impl Projection {
    ...
    /// # Errors
    ///
    /// Will return `Err` if `out` includes a `MetaVar` absent from `inp`.
...
fn make_projection(inp: &[MetaVar], out: &[MetaVar]) -> Result<Projection, &'static str> {
    ...
    for needle in out.iter() {
        ...
        indices.push(u8::try_from(index).expect("Wide predicates not supported"));
    }
...
impl MultiProjection {
    /// Returns a `MultiProjection` from `inp`s into `out`.
    ///
    /// The `MultiProjection` does not check that the input `Capture`s
    /// match the pattern defined in `inp`; the caller is responsible
    /// for enforcing that relationship, e.g., by matching `Capture`s
    /// with a join on common variables.
    ///
    /// # Errors
    ///
    /// Returns `Err` if `out` includes a `MetaVar` that is absent
    /// from all `inp`s.
...
fn make_multi_projection(
    ...
    for needle in out.iter() {
        ...
        indices.push((
            u8::try_from(input_id).expect("Wide input predicates not supported"),
            u8::try_from(index).expect("Wide output predicates not supported"),
        ));
    }
